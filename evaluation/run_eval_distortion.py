...
# add distorsion
distorsion_types = ['color_jitter', 'crop', 'blur', 'noise', 'jpeg_compress','SDEdit','SDEdit2']

img_paths = sorted(glob.glob(f"{args.output_dir}/*.png"))
if not os.path.exists(f"{output_dir}/distorted"):
    os.makedirs(f"{output_dir}/distorted")
for distorsion_type in distorsion_types:
    if not os.path.exists(os.path.join(f"{output_dir}/distorted", distorsion_type)):
        os.makedirs(os.path.join(f"{output_dir}/distorted", distorsion_type))
    apply_distorsion(img_paths, f"{output_dir}/distorted", distorsion_type)

# calculate accuracy
for distorsion_type in distorsion_types:
    print(distorsion_type)
    img_paths = sorted(glob.glob(f"{output_dir}/distorted/{distorsion_type}/*.png"))
    simple_decode(
        args.msg_bits,
        args.msgdecoder,
        img_paths,
